...
def demo_print(data):
    ...
    for text, val in data:
        print("{text}:{align} 0x{val} ({length} bytes)".format(text=text, align=((maxlen - len(text)) * " "),
                                                               val=bytes_to_hex(val), length=len(val)))
...
def demo_hash(variant="Ascon-Hash256", hashlength=32):  # HERE
